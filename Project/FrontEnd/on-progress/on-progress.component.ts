...
@Component({
  ...
  styleUrls: ['./on-progress.component.css']
})
export class OnProgressComponent implements OnInit{
  ...
  ngOnInit() {
    // this.dataSource.data = this.dataService.getTasksByStatus('Completed');
      this.fetchOnProgressTasks();
      this.dataSource.sort = this.sort;
      this.dataSource.paginator = this.paginator;
  }
  fetchOnProgressTasks() {
    const getdata = JSON.parse(sessionStorage.getItem('userDetails') || '{}');
    const managerId = getdata.empId;
    const status = 'OnProgress'; 
  
    const apiUrl = `https://localhost:7171/api/Admin/GetTaskByOnProgress?managerid=${managerId}&status=${status}`;
      ...
      if (result) {
        ...
        if (index !== -1) {
          ...
          if (TASK_DATA[index].status !== result.status) {
            TASK_DATA[index].status = result.status;
            this.dataSource.data = TASK_DATA.filter(task => task.status === 'On Progress'); // Update dataSource with filtered data
          }
        }
      }
    });
