...
export class CompletedComponent implements OnInit {
  ...
  fetchCompletedTasks() {
    // const managerId = 1003;
    const getdata = JSON.parse(sessionStorage.getItem('userDetails') || '{}');
    const managerId = getdata.empId; // Your manager ID
    const status = 'Completed'; // Change status to 'Completed'
  
    const apiUrl = `https://localhost:7171/api/Admin/GetTaskByCompleted?managerid=${managerId}&status=${status}`;
      ...
      if (result) {
        ...
        if (index !== -1) {
          ...
          if (TASK_DATA[index].status !== result.status) {
            TASK_DATA[index].status = result.status;
            this.dataSource.data = TASK_DATA.filter(task => task.status === 'Completed'); // Update dataSource with filtered data
          }
        }
      }
    });
